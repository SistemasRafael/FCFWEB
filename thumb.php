...
$foto = getFoto($hash);
$src = $foto->path;
error_log("*-*-*--*-*--*-*-*- Thumb solicitado para " . $src);
...
// exif_read_data solo soporta jpgs y tiffs
    ...
    switch ($imgtype) {
        case 1: // gif
            ...
            break;
        case 2: // jpeg
            $exifthumb = exif_thumbnail($src, $exwidth, $exheight, $extype);
            // Si extype no es 2 (jpg)
            // tocaron ver imagenes tipo 8 (tiff) en una memoria de un cliente...
            if ($exifthumb !== false && $extype == 2) {
...
//        $neww = $width * $fotoy / $height;
    ...
    error_log("thumb size: [" . $neww . "," . $newh . "]");
